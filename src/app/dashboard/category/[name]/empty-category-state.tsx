...
export const EmptyCategoryState = ({ categoryName }: { categoryName: string }) => {
    ...
    const { data } = useQuery({
        queryKey: ["category", categoryName, "hasEvent"],
        queryFn: async () => {
  ...
  body: JSON.stringify({
    ...
    return (
        <Card contentClassName="max-w-2xl w-full flex flex-col items-center p-6" className="flex-1 flex items-center justify-center">
            <h2 className="text-xl/8 font-medium text-center tracking-tight text-gray-950">
                Create your first event {categoryName} event
            </h2>
