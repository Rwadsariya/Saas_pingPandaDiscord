...
const EVENT_CATEGORY_VALIDATOR = z.object({
    ...
    color: z.string().min(1, "Color is required.").regex(/^#[0-9A-F]{6}$/i, "Color must be a valid color format."),
...
export const CreateEventCategoryModel = ({ children, containerClassName }: CreateEventCategoryModelProps) => {
                ...
                <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
                    <div>
                        ...
                        <p className="text-sm/6 text-gray-600">
                            Create a new category to organize events.
                        </p>
                    </div>
                        ...
                        <div >
                            ...
                            <div className="flex flex-wrap gap-3">
                                {COLOR_OPTIONS.map((premadeColor) => (
                                    <button key={premadeColor} type="button" className={cn(`bg-[${premadeColor}]`, "size-10 rounded-full ring-2 ring-offset-2 transition-all", color === premadeColor ? "ring-brand-700 scale-110" : "ring-transparent hover:scale-105")} onClick={() => setValue("color", premadeColor)}></button>
                                ))}
                            </div>
